 ...
 */
function tip_jar_wp_heath_check_stripe_test_webhook_signature( $health_checks ) {

	$saved_settings = get_option( 'tip_jar_wp_settings' );

	$webhook_signature = tip_jar_wp_get_saved_setting( $saved_settings, 'stripe_webhook_signing_secret_test_mode' );

	$webhook_validated = tip_jar_wp_validate_test_stripe_webhook( $webhook_signature );
	...
	if ( ! tip_jar_wp_is_site_localhost() ) {
		$unhealthy_array = array(
			'mode'               => 'live_site',
			'instruction'        => __( 'Your Test-Mode Stripe webhook is missing. This is required for Stripe to talk to your website in test mode. Setting up test mode is not required to accept real payments.', 'tip-jar-wp' ),
			'fix_it_button_text' => __( 'Set up Test Mode Webhook (Optional)', 'tip-jar-wp' ),
				...
				'steps'                          => array(
					...
					'step3' => array(
						...
						'input_field' => array(
							...
							'help_text'               => array(
								'title' => __( 'Stripe Webhook Signing Secret (Test Mode)', 'tip-jar-wp' ),
								// translators: First: Link to Stripe webhooks page. Second: the URL to point the webhook at.
								'body'  => sprintf( __( 'Create a webhook in [your Stripe Account](%1$s) pointing at this URL: %2$s. Then, copy and paste the "Signing secret" here.', 'tip-jar-wp' ), 'https://dashboard.stripe.com/account/webhooks', get_bloginfo( 'url' ) . '?tip\_jar\_wp\_stripe\_webhook' ),
							),
						),
					),
				),
		...
		$unhealthy_array = array(
			'mode'               => 'localhost',
			'instruction'        => __( 'Test Stripe Webhooks: It looks like you are on a localhost. Stripe will not be able to send webhooks to your website. Webhooks are very important and will only work on a live site.', 'tip-jar-wp' ),
	...
	$health_checks['stripe_test_webhook_signature'] = array(
		'priority'        => TIP_JAR_WP_WIZARD_TEST_MODE ? 300 : 600,
		...
		'healthy'         => array(
			'instruction'    => __( 'Your Stripe Test-Mode webhooks are all set up. Great work!', 'tip-jar-wp' ),
			'component_data' => array(
				'strings' => array(
					'title'                        => __( 'Stripe webhook successfully connected (Test Mode).', 'tip-jar-wp' ),
					'description'                  => __( 'Stripe can now "talk" back to your website when testing your payment form.', 'tip-jar-wp' ),
...
add_filter( 'tip_jar_wp_health_checks_and_wizard_vars', 'tip_jar_wp_heath_check_stripe_test_webhook_signature' );
