 ...
 * @return      bool
 */
function tip_jar_wp_send_receipt_email( $transaction ) {
	...
	$email_subject = sprintf( __( 'Your transaction receipt from %1$s. Transaction ID: %2$s', 'tip-jar-wp' ), get_bloginfo( 'name' ), $transaction->id );

	$email_message = tip_jar_wp_get_html_receipt( $transaction );
 ...
 * @return      bool
 */
function tip_jar_wp_get_html_receipt( $transaction = null ) {
	...
	<?php
	echo esc_textarea( tip_jar_wp_get_saved_setting( $saved_settings, 'tip_form_thank_you_message' ) );
	?>
	</div>
	<div class="tip-jar-wp-receipt" style="margin: 30px 0px 0px 0px;">
		...
		><?php echo esc_textarea( __( 'Your Receipt', 'tip-jar-wp' ) ); ?></div>
		...
		<div class="tip-jar-wp-receipt-statement-descriptor">
			...
			color: #000;
			text-shadow: 0 1px 0 #fff;"
			><?php echo esc_textarea( __( 'This will show up on your statement as:', 'tip-jar-wp' ) ); ?> </span>
