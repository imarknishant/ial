 ...
 * @return      bool
 */
function tip_jar_wp_send_refund_email_to_admin( $transaction ) {
	...
	$email_subject = sprintf( __( 'A refund has been given on %1$s. Transaction ID: %2$s', 'tip-jar-wp' ), get_bloginfo( 'name' ), $transaction->id );

	$email_message = tip_jar_wp_get_html_refund_for_admin( $transaction );
 ...
 * @return      bool
 */
function tip_jar_wp_get_html_refund_for_admin( $transaction = null ) {
	...
	<div class="tip-jar-wp-receipt" style="margin: 30px 0px 0px 0px;">
		...
		><?php echo esc_textarea( __( 'A refund was given', 'tip-jar-wp' ) ); ?></div>
