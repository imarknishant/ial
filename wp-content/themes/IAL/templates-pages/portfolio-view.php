...
$number_of_posts = count_user_posts($userid, 'videos', true );
                        ...
                        <div class="tab-pane fade show active" id="posts">
                            <div class="navTab_content">
                                ...
                                foreach($users as $usr){
                                    $u_id = $usr->ID;
                                    /*** Check if user have portfolio ****/
                                    $p_result = $wpdb->get_results("SELECT * FROM wp_ialposts WHERE post_author = $u_id AND post_type = 'portfolio'");
                              
                                    if(!empty($p_result)){
                                        $p_name = get_user_meta($usr->ID, 'first_name',true).' '.get_user_meta($usr->ID, 'last_name',true);
                                        $p_image = get_field('profile_image','user_'.$usr->ID);
                                        if(empty($p_image)){
                                            $p_image = 'https://ial.customerdevsites.com/wp-content/uploads/2021/04/avtar-img-1.png';
                                        }
                                        $p_url = get_author_posts_url($usr->ID);
                                    
                                    ?>
                                    <div class="video_block zoom_effect">
                                      <figure>
                                          <p class="figure_title"><?php echo $p_name; ?></p>
                                           <img src="<?php echo $p_image; ?>" alt="category-img-1.jpg" />
<!--                                          <a href="javascript:void(0);" class="material-icons">play_circle_filled</a>-->
                                      </figure>
                                       <div class="video_content">
                                         <h6><a href="<?php echo $p_url; ?>">View Portfolio</a></h6>
